...
class Life: NSObject {
    ...
    func fetchAsync() {
        if !isFetching, let url = weatherURL {
            print("start getting life \(self.interName)")
            isFetching = true
            let downloadTask = URLSession.shared.downloadTask(with: url,completionHandler: {
                [weak self] url, response, error in if let error = error {
                    print("error getting life \(self?.interName) error: \(error)")
                    ...
                    do {
                        ...
                        print("received life \(self?.interName)")
                        DispatchQueue.main.async {
                            if let strongSelf = self {
                    ...
                    print("failed getting life \(self?.interName) error: \(response)")
