...
class Now: NSObject {
    ...
    func fetchAsync() {
        if !isFetching, let url = weatherURL {
            print("start get now \(self.interName)")
            isFetching = true
            let downloadTask = URLSession.shared.downloadTask(with: url,completionHandler: {
                [weak self] url, response, error in if let error = error {
                    print("error get now \(self?.interName) error: \(error)")
                    ...
                    do {
                        ...
                        print("received data for now \(self?.interName)")
                        DispatchQueue.main.async {
                            if let strongSelf = self {
                                strongSelf.getData = getData as Data
                                print(getData)
                                NotificationCenter.default.post(name: NSNotification.Name(rawValue: "GetNow"), object: strongSelf)
                                strongSelf.isFetching = false
                            }
                        }
                    } catch {
                        print("error getting data for now \(self?.interName) error: \(error)")
                        DispatchQueue.main.async {
                            self?.isFetching = false
                        }
                    }
                } else {
                    print("failed get now \(self?.interName) error: \(response)")
