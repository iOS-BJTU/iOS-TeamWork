            ...
            let downloadTask = URLSession.shared.downloadTask(with: url,completionHandler: {
                [weak self] url, response, error in if let error = error {
                    ...
                    do {
                        ...
                        DispatchQueue.main.async {
                            if let strongSelf = self {
                                strongSelf.getData = getData
//                                try NSData(contentsOf: nowUrl!, options: NSData.ReadingOptions.uncached)
                                print(getData)
                                NotificationCenter.default.post(name: NSNotification.Name(rawValue: "ImageFetched"), object: strongSelf)
                                strongSelf.isFetching = false
                            }
                        }
                    } catch {
