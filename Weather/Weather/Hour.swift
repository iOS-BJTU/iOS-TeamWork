...
class Hour: NSObject {
    ...
    func fetchAsync() {
        if !isFetching, let url = weatherURL {
            print("start getting hour \(self.interName)")
            isFetching = true
            let downloadTask = URLSession.shared.downloadTask(with: url,completionHandler: {
                [weak self] url, response, error in if let error = error {
                    print("error getting hour \(self?.interName) error: \(error)")
                    ...
                    do {
                        ...
                        print("received hour \(self?.interName)")
                        DispatchQueue.main.async {
                            if let strongSelf = self {
                                strongSelf.getData = getData as Data
                                print(getData)
                                NotificationCenter.default.post(name: NSNotification.Name(rawValue: "GetHour"), object: strongSelf)
                                strongSelf.isFetching = false
                            }
                        }
                    } catch {
